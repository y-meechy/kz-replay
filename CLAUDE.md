...
No game install, no video rendering. The README explains the how and why in
depth; this file is the working knowledge you need to change the code safely.
...
node bin/kzreplay.js map kz_victoria # convert a map (needs local tools, see README)
```
...
- `deploy/`, `DEPLOYMENT.md` — Docker image and the GitHub Actions deploy.
...
- **A replay holds many attempts.** The run is the stretch from the *last*
...
- Comments explain *why* (the format quirk, the bug that forced the check),
...
Pushing to `main` deploys production: GitHub Actions runs the tests and then
triggers `deploy/deploy.sh` on the host over SSH. See `DEPLOYMENT.md`. A failed
or refused deploy shows up red in the Actions tab.
