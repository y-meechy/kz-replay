...
/**
 * Fetch the character once.
 *
 * Resolves to null rather than throwing when the file is not there: `ct.glb` is
 * generated by `npm run player-model` and, like a converted map, is not in the
 * repository. A checkout that has not run it still plays every replay — player.js keeps
 * its white ball. So a missing character is a normal state, not an error.
 */
export const loadCharacterAsset = (url = CHARACTER_URL) =>
  new Promise((resolve) => {
 ...
 */
export const disposeCharacterAsset = (asset) => {
